    ...
    <div className="px-8 py-6 max-w-[1400px] mx-auto">
      ...
      <div className="relative mb-8 overflow-hidden rounded-2xl border border-white/[0.08] bg-gradient-to-r from-[#161b22] to-[#1c2128]">
        ...
        <div className="relative px-8 py-10">
          ...
          <h1 className="text-3xl font-bold text-white mb-2">Hoş geldiniz, {user?.full_name?.split(' ')[0]}</h1>
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        <Link href="/projects" className="group relative bg-[#161b22] border border-white/[0.06] rounded-2xl p-5 hover:border-blue-500/20 hover:bg-white/[0.04] transition-all">
          <div className="flex items-start justify-between">
            <div>
              <p className="text-xs text-slate-500 font-medium mb-1">BOM Projeleri</p>
              <p className="text-2xl font-bold text-white">{stats.projects}</p>
            </div>
            <span className="text-2xl">📦</span>
          </div>
          <div className="mt-4 h-1 w-full rounded-full bg-gradient-to-r from-blue-600 to-blue-500 opacity-40" />
        </Link>
        <Link href="/tasks" className="group relative bg-[#161b22] border border-white/[0.06] rounded-2xl p-5 hover:border-purple-500/20 hover:bg-white/[0.04] transition-all">
          <div className="flex items-start justify-between">
            <div>
              <p className="text-xs text-slate-500 font-medium mb-1">Toplam Görev</p>
              <p className="text-2xl font-bold text-white">{stats.tasks}</p>
            </div>
            <span className="text-2xl">✅</span>
          </div>
          <div className="mt-4 h-1 w-full rounded-full bg-gradient-to-r from-purple-600 to-purple-500 opacity-40" />
        </Link>
        <div className="bg-[#161b22] border border-white/[0.06] rounded-2xl p-5">
          <div className="flex items-start justify-between">
            <div>
              <p className="text-xs text-slate-500 font-medium mb-1">Açık Görevler</p>
              <p className="text-2xl font-bold text-amber-400">{stats.openTasks}</p>
            </div>
            <span className="text-2xl">⏳</span>
          </div>
          <div className="mt-4 h-1 w-full rounded-full bg-gradient-to-r from-amber-600 to-amber-500 opacity-40" />
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="bg-[#161b22] border border-white/[0.06] rounded-2xl p-6">
          ...
          {loading ? (
            ...
            <div className="space-y-2">
              {projects.map((p: any) => (
                <Link key={p.id} href={`/project/${p.id}`} className="flex items-center gap-4 p-3 rounded-xl hover:bg-white/[0.04] transition-all group">
                  <div className="w-10 h-10 rounded-xl bg-blue-600/15 border border-blue-500/20 flex items-center justify-center text-blue-400 text-sm font-bold">{p.name?.charAt(0)}</div>
                  ...
                  <span className="text-[10px] text-slate-600">{p.createdAt ? new Date(p.createdAt).toLocaleDateString('tr-TR') : ''}</span>
                </Link>
              ))}
            </div>
          )}
        ...
        <div className="bg-[#161b22] border border-white/[0.06] rounded-2xl p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/tasks" className="text-xs text-purple-400 hover:text-purple-300">Tümünü gör →</Link>
          </div>
          {loading ? (
            <div className="py-8 text-center"><div className="w-6 h-6 border-2 border-blue-500 border-t-transparent rounded-full animate-spin mx-auto" /></div>
            ...
            <div className="space-y-2">
              {recentTasks.map((t: any) => {
                ...
                return (
                  <Link key={t.id} href="/tasks" className="flex items-center gap-4 p-3 rounded-xl hover:bg-white/[0.04] transition-all group">
                    <div className="w-10 h-10 rounded-xl bg-purple-600/15 border border-purple-500/20 flex items-center justify-center text-purple-400 text-sm font-bold">{t._count?.items || 0}</div>
                    <div className="flex-1 min-w-0">
